...
namespace AdventOfCode2021
{
	public class Day7
	{
		...
		public void Puzzle1(string crabInput, int expected)
		{
			...
			var highestCount = 0;
			var highestCrab = 0;
			var minCrab = int.MaxValue;
			var maxCrab = int.MinValue;
			foreach (var crab in crabs)
			{
				var next = 1;
				if (counts.TryGetValue(crab, out var existing))
				{
					next = existing + 1;
				}

				if (next > highestCount)
				{
					highestCount = next;
					highestCrab = crab;
				}
			...
			var minMovementCost = int.MaxValue;

			for (var c = minCrab; c <= maxCrab; c++)
			{
				var target = c;
				var movementCost = 0;
				foreach (var kvp in counts)
				{
					var crab = kvp.Key;
					var count = kvp.Value;
					movementCost += (count * Math.Abs(target - crab));
					if (movementCost >= minMovementCost) break;
				}
			...
			Assert.AreEqual(expected, minMovementCost);

		}

		[Test]
		[TestCase(INPUT, 91257582)]
		[TestCase("16,1,2,0,4,2,7,1,2,14", 168)]
		public void Puzzle2(string crabInput, int expected)
		{
			var crabs = crabInput.Split(",").Select(int.Parse).ToList();
			...
			var facHistory = new Dictionary<int, int>();
			int AddFac(int x)
			{
				if (x == 0) return 0;
				if (x == 1) return 1;
				if (facHistory.TryGetValue(x, out var old)) return old;

				var result = x + AddFac(x - 1);
				facHistory[x] = result;
				return result;
			}
			...
			for (var c = minCrab; c <= maxCrab; c++)
			{
				...
				foreach (var kvp in counts)
				{
					var crab = kvp.Key;
					var count = kvp.Value;
					var crabDist = Math.Abs(target - crab);
					var costPerCrab = AddFac(crabDist);
					if (crab < 4)
					{

					}
					movementCost += count * costPerCrab;
					if (movementCost >= minMovementCost) break;
				}
